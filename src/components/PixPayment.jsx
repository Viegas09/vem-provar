...
export default function PixPayment({ orderId, total, defaultEmail, onPaid, onCancel }) {
  ...
  if (!payment) {
    return (
      <form onSubmit={handleCreate} className="vp-fade-in" style={{ background: "#fff", border: `1px solid ${C.line}`, borderRadius: RADIUS.xl, padding: 20 }}>
        ...
        <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" required
          style={{ width: "100%", border: `1.5px solid ${C.line}`, borderRadius: RADIUS.sm, padding: "11px 14px",
        ...
        <button type="submit" disabled={creating}
          style={{ width: "100%", background: creating ? C.gray : C.orange, color: "#fff", border: "none", borderRadius: RADIUS.md,
  ...
  return (
    <div className="vp-fade-in" style={{ background: "#fff", border: `1px solid ${C.line}`, borderRadius: RADIUS.xl, padding: 20, textAlign: "center" }}>
      ...
      {status === "pending" && payment.pix?.qrCodeBase64 && (
        <img src={`data:image/png;base64,${payment.pix.qrCodeBase64}`} alt="QR code Pix"
          style={{ width: 220, height: 220, margin: "0 auto 14px", borderRadius: RADIUS.md, border: `1px solid ${C.line}` }} />
      )}
      {status === "pending" && (
        <>
          <button type="button" onClick={handleCopy} className="flex items-center justify-center gap-2"
            style={{ width: "100%", background: C.surface, border: `1px solid ${C.line}`, borderRadius: RADIUS.md, padding: "12px 0",
