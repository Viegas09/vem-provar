...
export default async function handler(req, res) {
  ...
  if (!code || !state || !UUID_RE.test(state)) {
    console.error("mp-oauth-callback: missing/invalid code or state", { hasCode: !!code, state });
  ...
  try {
    ...
    if (!tokenRes.ok || !tokenData.access_token) {
      console.error("mp-oauth-callback: token exchange failed", tokenRes.status, tokenData, { redirect_uri: `${origin}/api/mp-oauth-callback` });
      res.writeHead(302, { Location: `${dashboardUrl}?mp=error` });
      return res.end();
    }
